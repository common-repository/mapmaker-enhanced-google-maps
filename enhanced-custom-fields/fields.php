	...
	function set_value_from_input() {
		...
		foreach ($files_queue as $file) {
			...
			if ($this->allowed_extensions) {
				...
				if (!in_array($this->allowed_extensions)) {
...
class ECF_FieldPlantMap extends ECF_Field {
	function init() {
		wp_enqueue_script('plantmap', plugins_url('tpls/plantmap/func.js', __FILE__));
		ECF_Field::init();
	}
	...
	function render() {
		...
		$input_atts = $this->build_html_atts(array(
			...
			'name'=>$this->name,
			'value'=>$this->value,
			'class'=>'plantmap-coordinates',
		));
		$field_html = $this->build_tag('input', $input_atts);

		$size = @getimagesize(get_file_real_path(home_url() . get_option('cm_pin')));
		?>
			<style type="text/css">
			tr.ecf-plantmap td.plantmap .plantmap-canvas { height: 400px; cursor: crosshair; background: url(<?php echo home_url() . get_option('cm_image_big'); ?>) no-repeat left top; position: relative; overflow: hidden; border: 1px solid #cccccc; }
			tr.ecf-plantmap td.plantmap .plantmap-canvas.map-drag { cursor: move; }
			tr.ecf-plantmap td.plantmap .plantmap-canvas .plantmap-marker { display: block; position: absolute; width: <?php echo $size[0] ?>px; height: <?php echo $size[1] ?>px; background: url(<?php echo home_url() . get_option('cm_pin'); ?>); }
			</style>
			<tr class="ecf-field-container ecf-plantmap">
				<td class="ecf-label">
					<label><?php echo $this->label; ?></label>
				</td>
				<td class="plantmap">
					<?php if (!get_option('cm_pin') || !get_option('cm_image_big')) : ?>
						You need to upload a map and pin image in order to set the image coordinates for this map location.
					<?php else : ?>
						<div style="position: relative;">
							<div class="plantmap-canvas">
								<span class="plantmap-marker" data-coordinates="<?php echo $this->value; ?>"></span>
							</div>
							<p class="ecf-description">Click and drag to reposition the map. Double click to add marker position.</p>
							<?php echo $field_html; ?>
						</div>
					<?php endif; ?>
				</td>
...
class ECF_FieldMedia extends ECF_Field {
	...
	function render() {
		...
		$field_html .= '<input class="c2_open_media' . str_replace('-', '_', $this->id) .  ' button-primary" rel="media-upload.php?type=image" type="button" value="Select Media" />' . $image;
	...
	function add_correct_script_hooks() {
		wp_enqueue_script('utf8_decode_js_userialize', plugins_url('mapify_basic/lib/scripts/utf8.decode.js.unserialize.js', MAPIFYIT_PLUGIN_FILE));
		wp_enqueue_script('fancybox', plugins_url('lib/scripts/fancybox/jquery.fancybox-1.3.4.pack.js', MAPIFYIT_PLUGIN_FILE));
		wp_enqueue_style('fancybox-css', plugins_url('lib/scripts/fancybox/jquery.fancybox-1.3.4.css', MAPIFYIT_PLUGIN_FILE));
	}
	...
	function print_the_js() {
		...
		<script type="text/javascript">
			jQuery(function ($) {
				...
				$(document).on('click', '.c2_open_media<?php echo str_replace('-', '_', $this->id); ?>', function() {
					clicked = true;
